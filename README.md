...
- **匯出到文字框 / 從文字框匯入** — 直接複製貼上 JSON，適合快速搬資料
- **重設** — 清空回預設值
...
在「資料匯入/匯出」裡的**股價 API URL** 欄位填入 Worker 網址，例如：
...
1. 第一台裝置在「資料匯入/匯出」裡按 **產生新同步碼**（24 碼隨機字元）
2. 把這組碼抄到其他裝置的「同步碼」欄位
3. 三台裝置就共用雲端上的同一份資料
